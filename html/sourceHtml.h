...
xhttp.onreadystatechange=function(){if(this.readyState==4&&this.status==200){var resText=xhttp.responseText;var obj=JSON.parse(resText);console.log(resText);var revCls=document.querySelectorAll("[id]");var i;for(i=0;i<revCls.length;i++){var elm=revCls[i];if(elm.type=="checkbox")elm.checked=(obj[elm.id]==('true'));else if(elm.tagName=="label")elm.innerHTML=obj[elm.id];else if(elm.getAttribute("class")=="timePicker"){var m=elm.previousElementSibling;var h=m.previousElementSibling.previousElementSibling;var val=parseInt(obj[elm.id]);h.value=parseInt(val/60);m.value=val%60;elm.innerHTML='OK'}
