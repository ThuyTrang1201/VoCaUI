...
char * java_script = QUOTE(function isConnectedFunc(){document.getElementById("wifi").style.background="gray"}
function sendData(x){var formData=new FormData();if(x.type=="checkbox"){if(x.checked)formData.append(x.id,!0);else formData.append(x.id,!1)}else{formData.append(x.id,x.value)}
...
function onMenuClick(name){var divs=document.getElementsByTagName("div");for(var i=0;i<divs.length;i++){if(divs[i].id!=name&&divs[i].id!="menu-button"&&divs[i].id!="wifi"&&divs[i].id!="menu"&&divs[i].id!="mask")
...
reciveClass[i].checked=obj[reciveClass[i].id];else if(reciveClass[i].tagName=="LABEL")
reciveClass[i].innerHTML=obj[reciveClass[i].id];else reciveClass[i].value=obj[reciveClass[i].id]}
...
var d=new Date(0);d.setUTCSeconds(obj.time);document.getElementById("time").innerHTML=d.toISOString().split("T")[1].split(".")[0];ajaxData()}};xhttp.open("GET","/transData",!0);xhttp.send()}
